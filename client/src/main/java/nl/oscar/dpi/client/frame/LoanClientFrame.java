...
public class LoanClientFrame extends JFrame {
    ...
    private RequestReply<LoanRequest, LoanReply> getRequestReply(String uuid) {

        for (int i = 0; i < listModel.getSize(); i++) {
            RequestReply<LoanRequest, LoanReply> rr = listModel.get(i);
            if (rr.getRequest().getUuid().toString().equals(uuid)) {
                return rr;
            }
        }
    ...
    private void initReceiver() {
        receiver.setListener(new ObjectMessageListener<LoanReply>() {
            ...
            protected void receivedObjectMessage(LoanReply object, ObjectMessage message) {
                try {
                    RequestReply<LoanRequest, LoanReply> rr = getRequestReply(message.getJMSCorrelationID());
                    rr.setReply(object);
                    EventQueue.invokeLater(requestReplyList::repaint);
                } catch (Exception e) {
                    System.out.println(e.getMessage());
                }
            }
